...
def getPersonList():
    ...
    if searchChoose:
        query['$or'] = [{'personnel_name': {'$regex': searchChoose}},
                        {'personnel_social_card': {'$regex': searchChoose}},
                        {'personnel_number': {'$regex': searchChoose}}]
...
def validPersonImage():
    ...
    #   mainlogger.info('---nginx_port : %s'%port)
...
bp.add_url_rule(('/{}/<string:{}>').format('face_images', 'image_id'), view_func=view_func,
                methods=['GET', 'PUT', 'DELETE'])

view_func_feature = FaceFeatureDBAPI.as_view(('{}_api').format('face_features'))
bp.add_url_rule(('/{}/<string:{}>').format('face_features', 'image_id'), view_func=view_func_feature,
                methods=['GET', 'PUT', 'DELETE'])
