...
from edgebox_db.mongo_collections import WORK_FLOW_MISSION
...
def getEmergencyRecordByPositionCity():
    ...
    emergencyRecordList = []
    for item in res:
        #  mainlogger.info("item:%s"%item)
        newItem = {}
        model_path = item["_id"]
        count = item["count"]
        newItem["modelPath"] = model_path
        newItem["times"] = count
        totalTypeCount = my_db.get_aggregate("odin_business_emergency_record",
                                             query={"emergency_time": {"$regex": todayStr}, "model_path": model_path})
