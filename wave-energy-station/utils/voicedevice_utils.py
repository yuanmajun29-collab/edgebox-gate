...
def sendSoundmessage(sound_col, equip_id, sound_type, sound_file, server_url=None, account=None, password=None,
                     tts_url=None):
    try:
        ...
        for sound_id in sound_id_list:
            ...
            if device_type == '1':
                ...
                if sound_type == 1:
                    ...
                    if tts_result.get('result') != 200:
                        mainlogger.info('Error : 播放tts失败!')
                elif sound_type == 2:
                    # 音频文件
                    music_result = VoiceInstance.playMusic(endPointIDsList=[1], musicIDsList=[sound_file])
                    music_result = music_result.json()
                    if tts_result.get('result') != 200:
                        mainlogger.info('Error : 播放音乐失败!')
            elif device_type == '2':
                # 设备类型为菱声音响
            ...
            mainlogger.info("sendSoundmessage  tts_result%s" % tts_result)
    except Exception as e:
        mainlogger.info('Error : 声光告警失败, Reason: %s' % e)
