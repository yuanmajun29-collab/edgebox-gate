...
from Utils.utils import generate_log,set_success_result,set_fail_result
...
def account_phone_pwd_login():
    ...
    accountOrPhone = params.get("accountOrPhone", None)
   ...
   # generate_log(request,db=my_db)
    ...
    error_response['errorCodeDesc'] = "错误的账号密码"
    
    user = user_coll.find_one({ 'user_status':'1','$or': [{'user_account':accountOrPhone},{'user_phone':accountOrPhone}]})
    if not user:
        # 错误账号
...
def get_permission_list():
    ...
    '''
    my_db = ToMongo('wavedevice')
    permission_coll = my_db.get_col('authority_permission').find({})
    infolist =[]
