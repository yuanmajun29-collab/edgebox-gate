...
from config import SYSTEMLOGO_URL,NetAgreementType
...
def updateSystemSetting():   
    ...
    res ={'emergency_pop_state':emergency_pop_state,'system_name':system_name}
...
def getCunrrentIp():   
    ...
    try:
        ...
        err1,result1 = execShell("sudo ifconfig eth1")
        ...
        if err1 == 1:
            ...
            item1['ipv4Broadcast'] = result1[9]
            item1['ipv4Inet'] = result1[5]
            item1['ipv4netmask'] = result1[7]
            data.append(item1)
    except Exception as e:
...
def setWorkModel():   
    ...
    if not serviceAddress:
        ...
        remote_url = NetAgreementType + "://" + server_ip + ":" + str(server_port)
    else:
        remote_url = NetAgreementType + "://" + serviceAddress + ":" + str(servicePort)

    register_item,content = query_edge_service(my_db)
    register_item['account'] = serviceAccount
    register_item['pwd'] = servicePwd

    if model == "0":
        #向服务端更改盒子模式
        stateItem = {"serviceState":"1","deviceSn":register_item['deviceSn']}
        response = switch_state(stateItem,remote_url)
        mainlogger.info("--switch_state content: %s"%stateItem)
        if not response:
            error_response['errorCodeDesc'] = "切换模式接口无响应"
            return jsonify(error_response)
        if response.get("code") == 200:
            #成功变更为单机模式，则删除全部数据
            #更新模式为单机模式
            my_db.update("authority_work_model",{},{"$set":{"model":model}})
            ans = deleteAllData(my_db)    
            sender.send_reboot_message()
        else:
            error_response['errorCodeDesc'] = "切换模式接口错误"
            return jsonify(error_response)
    ...
    mainlogger.info("--register content: %s"%register_item)
    if response['requestStatus'] == "SUCCESS":
        ...
        #调用同步接口，同步数据
        try:
            res_data = sync_all_info(remote_url,content)
            res_version = sync_heartbeat(remote_url,content)
        except:
            error_response['errorCodeDesc'] = "向云平台发起数据同步失败"
            return error_response

        data = res_data.get("data")
        insert_all_info(my_db,data)

        sender.send_reboot_message()

        #同步数据版本
        versionList = res_version['data']
