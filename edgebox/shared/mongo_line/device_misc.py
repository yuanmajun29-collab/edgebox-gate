...
#from sophonface import Product
Product = 'SE5'
...
def make_error_response(error_type, message="", chinese_message=""):
    ...
    if error_type == ErrorResponseType.URL_INVAL:
        ...
        if Product == "SE3":
            message = 'Device must <= {}(photo = 0.5, other = 1.0),Or 10 photo devices'.format(Max_support_device_num[Product])
            chinese_message = '设备数量必须小于等于{}路(photo = 0.5, other = 1.0),或是支持10路图片流设备'.format(Max_support_device_num[Product])
        else:
            message = 'Device must <= {}(photo = 0.5, other = 1.0)'.format(Max_support_device_num[Product])
            chinese_message = '设备数量必须小于等于{}路(photo = 0.5, other = 1.0)'.format(Max_support_device_num[Product])
