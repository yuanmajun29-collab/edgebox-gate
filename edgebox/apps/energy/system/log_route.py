...
def SysResources():
    ...
    if not sys_item:
        item = {'device_id': None, 'device_name': None, 'organization_id': organization_id,
                'alarm_storage_location': alarmStorageLocation, 'disk_space': diskSpace,
                'sms_notification_account': smsNotificationAccount,
        ...
        item_update = {'alarm_storage_location': alarmStorageLocation, 'disk_space': diskSpace,
                       'sms_notification_account': smsNotificationAccount,
                       'email_notification_account': emailNotificationAccount, 'modify_by': current_userid,
                       'modify_time': now}
...
def uploadFile():
    ...
    if filetype == "rar":
        ...
        mainlogger.info("upgradeAlg  文件类型不支持")
