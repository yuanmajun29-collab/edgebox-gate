...
import sys
sys.path.append("/data/ebox/wavegate/wave-ai-spirit/Utils")
...
def setup_logging(default_path='/data/ebox/wavegate/wave-ai-spirit/Utils/logging.yaml', default_level=logging.INFO):
    path = default_path
...
def init_logger(logger):
    setup_logging("/data/ebox/wavegate/wave-ai-spirit/Utils/logging.yaml")

def initialize():
    global rootLoggerInitialized
    with initLock:
        if not rootLoggerInitialized:
            filetime = os.path.getmtime("/data/ebox/wavegate/wave-ai-spirit/Utils/logging.yaml")
