...
def queryRoi():
    ...
    result = my_db.get_col("odin_device_roi_area_record").find({"camera_id":cameraId,"roi_type":roiType},{"_id":0})
    if result:
        for res in result:
            iter = {}
...
def addRoi():
  ...
  #  roi_id = params.get("roiId",None)
    ...
    res_item = {"camera_id":camera_id,"roi_area_info":json.dumps(roi_area_info),"roi_name":roi_name,"algorithm_constant_id":algorithms_id,
           "roi_id":num_max+1,"create_time":create_time,"roi_area_record_id":roi_area_record_id,"organization_id":organization_id,"roi_type":roiType}    
