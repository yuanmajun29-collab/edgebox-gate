...
from device.konad_service import insert_to_database,sync_device_status,sync_emergency,sync_temperature
...
def send_out(udp_socket, recv_ip, sn, functionCode,temp_max,temp_min):
    ...
    if functionCode == b'\x01' or functionCode == b'\x0B':
        ...
        CRCM = bytearray.fromhex((calc_crc((b'\x7e' + sn + functionCode + dLen + state + cLen + cdata).hex()))) #计算CRC校验码
...
def deal_msg(db:ToMongo,rePool,sn,temperature,mode,url):
    try:
        ...
        if mode == "1":
         ...
         #   mainlogger.debug('--content:%s'%content)
            sync_emergency(url,content)

    except Exception as e:
        mainlogger.debug("温度告警器deal_msg error:%s"%traceback.format_exc())
...
class TempServer():
    ...
    def __init__(self,context):     
        if not  TempServer.__flag:
            ...
            mainlogger.debug("温度传感器进程绑定 IP 0.0.0.0 , 端口 5555")
    ...
    def set_temp_threhold(self,temp_max,temp_min):
        ...
        mainlogger.debug("设置温度阈值,max:%s,min:%s,socketMacMap:%s"%(temp_max,temp_min,self.socketMacMap))
        for ip in keys:
    ...
    def recv_data(self):
        while True:
            ...
            if data[0:1] == b'\x7e' and data[len(data)-1:len(data)] == b'\x0D': # 判断是否包头为7E,包尾为0D
             #   mainlogger.debug("--接收温度传感器数据:%s,传感器IP:%s"%(data.hex(),recv_ip))
                snStr = sn.hex()
                ...
                if functionCode == b'\x01' or functionCode == b'\x0B':                  
                    ...
                    if emergencyCode != b'\x01' and emergencyCode != b'\x02':  # '\x01' 和 '\x02' 表示高温告警和低温告警  
                        continue                    
                    # 根据SN号判断设备类型,SN号C0开头为,80开头为单温度
                  #  mainlogger.debug('----emergencyCode : %s'%emergencyCode)
                    
                    if sn[0:1] == b'\xC0':
                        mainlogger.debug("--接收温度传感器数据解析:SN号:%s,温度:%s,湿度:%s"%(snStr,temperature,humidity))
    ...
    def report_temp(self):
        try:
            ...
            mainlogger.debug("--温度上报error:%s"%e)
        finally:
    ...
    def check_status(self):
        try:
        #    mainlogger.debug("--self.socketMacMap:%s"%self.socketMacMap)
            ...
            #    mainlogger.debug("--动环设备状态：%s,mac:%s,ip:%s"%(status,mac,ip))
            ...
            mainlogger.debug("温度告警器check_status_error :%s"%e)
        finally:
