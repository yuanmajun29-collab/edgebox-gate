...
from system.dynamic_misc import insert_dynamic_info
...
REGISTER_DEVICE = "/device/syn/registerDevice"

HEART_BEAT ="/device/syn/heartBeat"
SYN_PERSONNEL_INFO ="/business/syn/synPersonnelInfo"
SYN_DEVICE_POSITION="/device/syn/syncDevicePosition"
SYN_DEVICE_CAMERA="/device/syn/syncDeviceCamera"
SYN_CONTROL ="/business/syn/syncControlItem"
SYN_ALG_INFO = "/business/syn/synAlgorithmConstant"
SYN_DYNAMIC_INFO = "/device/syn/synDynamic"
...
def request_login_url(params):
    ...
    remote_url = "http://" + serviceAddress + ":" + servicePort
...
def deleteAllData(my_db:ToMongo):
    ...
    #删除弯道相关的表
    my_db.delete("corner_overtaking_config",{},is_one=False)
    my_db.delete("corner_overtaking_project",{},is_one=False)
    my_db.delete("corner_overtaking_emergency_record",{},is_one=False)
...
def get_constant_entity(my_db:ToMongo):
    try:
        ...
        mainlogger.debug("get_constant_entity error :%s"%e)
        entities = []
    return entities
...
def register_device(serviceState,remote_url,content):
    ...
    try:
        ...
        answer = answer.json()
        mainlogger.debug("--切换模式response-- :%s"%answer)
        if answer['code'] == 200:
            return success_result
    except Exception as e:
        mainlogger.debug("--切换模式失败 :%s"%e)
...
def convert_control_format(msg):
    ...
    flowMissionSyncVoList = msg.get('flowMissionSyncVoList')
    controlTaskEntityList = msg.get('controlTaskEntityList')
    flowMissionGroupVoList = msg.get('flowMissionGroupVoList')
    ...
    res['cornerOvertakingProject'] = msg.get('cornerOvertakingProject')
    res['cornerOvertakingConfigList'] = msg.get('cornerOvertakingConfigList')

    res['trafficLightRuler'] = msg.get('trafficLightRuler')
    res['trafficLightConfigList'] = msg.get('trafficLightConfigList')
    res['trafficLightProject'] = msg.get('trafficLightProject')
...
def get_alg_status():
    if redis_database.exists('alg_status'):
        status = '0'
    else:
        status = '1'
    return status
...
class SyncDataTimer():
    ...
    def query_edge_version(self):
        try:
            ...
            content['algStatus'] = get_alg_status()
            mainlogger.debug("--HeartBeat Content:%s"%content)
            data = requests.post(url,data=json.dumps(content),headers=headers,verify=False)
            data = data.json()
            dataDict = data.get("data",[])
            systemDataVersionList = dataDict.get('list')
            bindOrganizationId = dataDict.get("bindOrganizationId")

            if bindOrganizationId and bindOrganizationId != self.bind_organization_id:
                # 平台上，算力盒所属组织发生改变
                self.bind_organization_id = bindOrganizationId
                self.my_db.update('authority_work_model',{},{'$set':{'service_organization_id':bindOrganizationId}})
            ...
            mainlogger.debug("query_edge_version error:%s"%e)
            ans = {}
        return ans
    ...
    def check_version_func(self):
        ...
        while self.work_model == '1':
            ...
            mainlogger.debug("--平台的data version:%s"%edge_version)
            if not edge_version:
    ...
    def check_adress_thread(self,box_version,edge_version,content):
        if box_version['address'] < edge_version['address']:
            ...
            PositionList = data_position['data']
    ...
    def check_control_thread(self,box_version,edge_version,content):
        mainlogger.debug("=========任务信息同步云平台======{}, {} {}".format(box_version, edge_version, content))
        if box_version['control'] < edge_version['control']:
            #任务信息同步云平台
            mainlogger.debug("=========任务信息同步云平台进行中======{}, {} {}".format(box_version, edge_version, content))
            resp = sync_control_info(self.remote_url,content)
            data_control = resp.get("data")
            controlManageEntity = convert_control_format(data_control)
            mainlogger.debug("=========任务信息同步controlManageEntity======{}".format(controlManageEntity))
    ...
    def check_device_thread(self,box_version,edge_version,content):
        if box_version['device'] < edge_version['device']:
            ...
            cameraSyncRepVOList = data_device.get('data')
            result = insert_camera_info(self.my_db,cameraSyncRepVOList)
    ...
    def check_personnel_thread(self,box_version,edge_version,content):
        if box_version['personnel'] < edge_version['personnel']:
            #人脸库信息同步云平台
            resp = sync_personn_info(self.remote_url,content)
            data_person = resp.get("data")
    ...
    def check_alg_thread(self,box_version,edge_version,content):
        ...
        if box_alg_version < edge_alg_version:
            #算法信息同步云平台
            resp = sync_alg_info(self.remote_url,content)
            algorithmConstantEntityList = resp.get("data")
    ...
    def check_dynamic_thread(self,box_version,edge_version,content):
        ...
        if box_dynamic_version < edge_dynamic_version:
            #动环信息同步云平台
            resp = sync_dynamic_info(self.remote_url,content)
            dynamic_info = resp.get("data")
...
class SyncDeviceSrs():
    ...
    def pushStreamToSrs(self,cameralist,srsurl):
        ...
        try:
            mainlogger.debug("start push camera stream to srs,srsUrl:%s"%srsurl)
            for cameraInfo in cameralist:
                camera_id = cameraInfo['camera_id']
                pid = getPid(command=camera_id,Toflag="ffmpeg")
                if pid:
                    mainlogger.debug("摄像头:%s推流进程已存在,不再重复推送"%camera_id)
                    continue
                rtspUrl = cameraInfo.get("live_url",None)
                if not rtspUrl:
                    mainlogger.debug("摄像头:%s的rtspUrl为空,不推送"%camera_id)
                    continue
                ...
                mainlogger.debug("push stream command:%s"%command)
                error,response = execShell(command)
        except Exception as e:
            mainlogger.debug("摄像机推送流至srs失败%s"%e)
    ...
    def syncStatusAndPushSrs(self):
        ...
        try:
            ...
            if model == "0" or model == None:
                mainlogger.debug("盒子工作模式为单机,不同步摄像头状态至云平台,也不推送srs")
                return
            mainlogger.debug("start syncStatus snd push srs")
            ...
            remote_url = "http://" + workmodel_info['service_address'] + ":" + workmodel_info['service_port'] + "/device/syn/synCameraStatus"
            mainlogger.debug("sync camera status url:%s", remote_url)

            if len(offline_ids) != 0:
                mainlogger.debug("摄像机下线同步至云平台")
                resp = self.httpToPost(remote_url,offline_ids,cameraStatus=1)
                mainlogger.debug("摄像机下线同步至云平台resp:%s"%resp)
            ...
            if len(online) != 0:
                ...
                mainlogger.debug("摄像机上线,摄像机视频流至SRS,size:%s,srsUrl:%s"%(len(online),srsUrl))
                self.pushStreamToSrs(online,srsUrl)
                mainlogger.debug("摄像机上线同步至云平台")
                response = self.httpToPost(remote_url,self.online_ids,cameraStatus=0)
                mainlogger.debug("摄像机上线同步至云平台resp:%s"%response)

        except Exception as e:
            mainlogger.debug("摄像机推送流至srs失败 error:%s"%e)
        ...
        try:
            for camera_id in self.online_ids:
                ...
                if pid :
                    ...
                    mainlogger.debug("关闭摄像头:{}ffmpeg推流进程成功，PID:%s"%pid)
        except Exception as e:
...
class SyncDeviceP2p():
    ...
    def httpRequestGetP2pAccount(self,url,organizationId,bindOrganizationId):
        ...
        map['cpuSn'] = baseinfo_item.get('equipment_model',None)
        headers = {"Content-Type": "application/json"}
    ...
    def getStreamList(self,newCameraInfo,p2pAccount,syncNoUrl,maxVideoId):
        ...
        try:
            ...
            mainlogger.debug("sync camera number result:%s"%resp.json())
            mainlogger.debug("sync camera number param:%s"%param)
        except Exception as e:
            mainlogger.debug("getStreamList Error:%s"%e)
    ...
    def scanCameraUpdateP2pConfig(self):
        ...
        try:
            model = workmodel_info['model']
            if model == "0" or model == None:
                mainlogger.debug("盒子工作模式为单机,不同步摄像头状态至云平台")
                pid = getPid(command="pgRtspLiveCapSvc",Toflag="pgRtspLiveCapSvc")
                if pid:
            #        closeLinuxProcess(pid)
                    mainlogger.debug("p2p进程存在，kill p2p 进程")
                return
            ...
            if not os.path.exists(p2pExecuteFilePath) or not os.path.exists(jsonConfigFilePath) or not os.path.exists(cfgConfigFilePath):
                mainlogger.debug("p2p可执行程序或配置文件不存在，不做处理")
                return
            camera_col = self.my_db.get_col("odin_device_camera_edit")
            ...
            if camera_infos.count() == 0:
                mainlogger.debug("没有可以配置的摄像头，不做处理")
            ...
            #联网模式下向平台同步摄像机状态
            online_ids = camera_col.distinct('camera_id',{'camera_status':'0'})
            offline_ids = camera_col.distinct('camera_id',{'camera_status':'1'})
            syncUrl = NetAgreementType + "://" + workmodel_info['service_address'] + ":" + workmodel_info['service_port'] + "/device/syn/synCameraStatus"
            if len(online_ids) != 0:
                resp = self.syncCameraStatus(syncUrl,online_ids,cameraStatus=0)
                mainlogger.debug("摄像机上线同步至云平台resp:%s"%resp)
            if len(offline_ids) != 0:
                resp = self.syncCameraStatus(syncUrl,offline_ids,cameraStatus=1)
                mainlogger.debug("摄像机下线同步至云平台resp:%s"%resp)
                   
            #获取当前盒子的p2p账号和p2p服务地址
            remote_url = NetAgreementType +"://" + workmodel_info['service_address'] + ":" + workmodel_info['service_port'] + "/device/syn/getCtP2pAccountInfo"
            ...
            serverAddr = data.get('serverAddr',None)
            if not p2pAccount or not serverAddr:
                mainlogger.debug("当前盒子未配置p2p账号或服务地址")
                return
            ...
            if len(oldCameraInfo) == len(newCameraInfo) and pid and list(oldCameraInfo.values()) == list(newCameraInfo.values()) and p2pAccountBefore == p2pAccount:
                mainlogger.debug("盒子摄像头配置未变动，p2p账号未更改，不做处理")
                return
            
            #先杀p2p进程
            if pid:
                mainlogger.debug("kill p2p 进程，准备写入配置文件")
                closeLinuxProcess(pid)
            
            mainlogger.debug("start write  p2p config")
            syncCameraNoUrl = NetAgreementType + "://" + workmodel_info['service_address'] + ":" + workmodel_info['service_port'] + "/device/syn/syncCameraNo"
