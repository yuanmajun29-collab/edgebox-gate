...
from Utils.device_misc import make_response, make_error_response, ErrorResponseType, make_success_response
from Utils.db import ToMongo
import uuid
from Utils.utils import generate_log
from Utils.jwt_verify import *
from Utils.emergencydb import delete_pic,EventImageDBAPI,transfer_img_url
...
def getEmergencyDetailInfo():
    ...
    my_db = ToMongo("wavedevice")
    generate_log(request,db=my_db)
    emergency_info = my_db.get_col("odin_business_emergency_record").find_one({"emergency_record_id":emergencyRecordId},{"_id":0})
    detail_info = my_db.get_col("odin_business_emergency_record_detail_info").find_one({"emergency_record_id":emergencyRecordId},{"_id":0})
    if emergency_info:
        control_info = my_db.get_col("odin_business_control_manage").find_one({"control_id":emergency_info['mission_id']},{"_id":0})
    else:
        control_info = None
...
def saveEmergencyExecute():
    ...
    item = {"emergency_exec_desc":emergencyExecDesc,"emergency_exec_name":emergencyExecName,"emergency_exec_result":emergencyExecResult}
    if  is_wrong:
...
bp_alert.add_url_rule(('/{}/<string:{}>').format('event_images', 'image_id'), view_func=view_func,methods=['GET', 'PUT', 'DELETE'])
