...
class Sendwebrequest():
    ...
    def get_webhook_delivery(self):
        ...
        for relay_item in relay_col:
            ...
            frequency_type = relay_item['frequency_type']
            frequency_date_format = relay_item['frequency_date_format']
            frequency_count = relay_item['frequency_count']
            ...
            relay_item['frequency_type'] = frequency_type
            relay_item['frequency_date_format'] = frequency_date_format
            relay_item['frequency_count'] = frequency_count
    ...
    def send_webhook_thread(self,msg,param_dict): 
        if self.delivery_task:
            ...
            for delivery in self.delivery_task:
                if delivery['frequency_type'] == '1':
                    delivery_thread = Thread(target=self.send_web_delivery,args=[delivery,msg,param_dict])
                    delivery_thread.start()
                if delivery['frequency_type'] == '2':
                    if delivery['frequency_date_format'] == '1':
                        delayTimes = int(delivery['frequency_count']) * 60
                    if delivery['frequency_date_format'] == '2':
                        delayTimes = int(delivery['frequency_count']) * 60 * 60
                    if delivery['frequency_date_format'] == '3':
                        delayTimes = int(delivery['frequency_count']) * 60 * 60 * 24

                    check_thread = threading.Timer(delayTimes, self.send_web_delivery,args=[delivery,msg,param_dict])
                    check_thread.start()
    ...
    def send_web_delivery(self,delivery,msg,param):
        ...
        mainlogger.debug('--告警转发内容 :   %s'%relay_copy)
        ...
        try:
            ...
            if response.status_code == requests.codes.ok:
                relay_status = '1'
                mainlogger.debug("==告警转发成功==")
            else:
                relay_status = '2'
                mainlogger.debug("==告警转发失败,result code:{}==".format(response.status_code))

        except Exception as e:
            relay_status = '2'
            mainlogger.debug("==告警转发出现错误,请检查网络或转发地址是否正确！error:%s"%e)
