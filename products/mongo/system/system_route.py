...
from config import SYSTEMLOGO_URL,SHELL_DIR
...
def updateSystemSetting():   
    ...
    res = {'emergency_pop_state':emergency_pop_state,'system_name':system_name,'home_overview_style':homeOverviewStyle}
...
def saveOrUpdate4():   
    ...
    cmd_str = SHELL_DIR + 'bm_set_ip eth0 %s %s %s %s' % (IPv4_address , IPv4_mask , IPv4_gateway , dns_server)
...
def getCunrrentIp():   
    ...
    try:
        ...
        if eth1_res:
            item1['ipv4Broadcast'] = eth1_res[2]
            item1['ipv4Inet'] = eth1_res[0]
            item1['ipv4netmask'] = eth1_res[1]
        else:
        ...
        mainlogger.debug("--getCunrrentIp error :%s"%e)
        data = []
...
def modifyCunrrentIp():   
    ...
    bash_set_ip = SHELL_DIR + "bm_set_ip"
...
def testWorkModel():   
    ...
    response_data,organization_id,bind_organization_id = request_login_url(params)
...
def setWorkModel():   
    ...
    my_db = ToMongo('wavedevice')
    kndInstance = KNDserver(context=None)
    ...
    if not serviceAddress:
        server_item = my_db.get_col("authority_work_model").find_one()
        server_ip = server_item['service_address']
        server_port = server_item['service_port']
        remote_url = "http://" + server_ip + ":" + str(server_port)
    else:
        remote_url = "http://" + serviceAddress + ":" + str(servicePort)

    register_item,content = query_edge_service(my_db)
    ...
    if model == "0":
        ...
        #向服务端更改盒子模式
        item = {"serviceState":"1","serialNumber":register_item['serialNumber'],"ip":register_item['ip']}
        server_item = my_db.get_col("authority_work_model").find_one()
        item['organizationId'] = server_item.get('organization_id')

        response = register_device(serviceState="1",remote_url=remote_url,content=item)
        mainlogger.debug("--register content: %s"%item)
       # register_item['organizationId']
        if response['requestStatus'] == "SUCCESS":
            #成功变更为单机模式，则删除全部数据,清除推流进程
          #  srs_instance.killCameraPushProcess()
            kndInstance.mode = "0"
            tempInstance.mode = "0"
            ans = deleteAllData(my_db)    
            sender.send_reboot_message()    
        #复原数据库data_version
        init_data_version(my_db)    
        return jsonify(response)

    #联网模式---向云平台发起登录请求
    response_data,organization_id,bind_organization_id = request_login_url(params)
    sync_res = sync_organizationId(my_db,organization_id,bind_organization_id)
    register_item['organizationId'] = organization_id
    register_item['bindOrganizationId'] = bind_organization_id
    content['organizationId'] = bind_organization_id if bind_organization_id else organization_id
    heartbeat_content = {"deviceSn":content['cpuSn'],"organizationId":organization_id,'bindOrganizationId':bind_organization_id}
    requestStatus = response_data.get("requestStatus",None)

    if requestStatus == "SUCCESS":
        #发起注册请求
        response = register_device(serviceState="0",remote_url=remote_url,content=register_item)
        mainlogger.debug("--register content: %s"%register_item)
        if response['requestStatus'] == "SUCCESS":
            #成功变更为联网模式，则删除全部数据
            ans = deleteAllData(my_db) 
            ...
            #调用同步接口，同步数据
            try:
                res_camera = sync_camera_device(remote_url,content)
                res_control = sync_control_info(remote_url,content)
                res_person = sync_personn_info(remote_url,content)
                res_position = sync_position_info(remote_url,content)
                res_dynamic = sync_dynamic_info(remote_url,content)
                res_version = sync_heartbeat(remote_url,heartbeat_content)

                res_control = res_control.get("data")
                res_person = res_person.get("data")
                res_dynamic = res_dynamic.get("data")

            except:
                error_response['errorCodeDesc'] = "向云平台发起数据同步失败"
                return error_response

            #同步成功后，插入到本地数据库
            cameraSyncRepVOList = res_camera['data']
            insert_camera_info(my_db,cameraSyncRepVOList)

            controlManageEntity = convert_control_format(res_control)
            insert_control_info(my_db,controlManageEntity)
            minio_addr = res_control.get('ipAddress',None)
            update_minio_address(my_db,minio_addr)

            PositionList = res_position['data']
            insert_position_info(my_db,PositionList)

            insert_person_info(my_db,res_person)
            insert_dynamic_info(my_db,res_dynamic)

            sender.send_reboot_message()

            #同步数据版本
            data = res_version['data']
            versionList = data.get('list')
            insert_version_info(my_db,versionList)
            ...
            kndInstance.get_mode()
            tempInstance.get_mode()
        ...
        error_response['errorCodeDesc'] = "向云平台发起登陆请求失败"
