    ...
    def send_web_delivery(self,delivery,msg,param):
        ...
        if "modelPath" in trigger_condition.keys():
            if msg['modelPath'] not in  trigger_condition['modelPath']:
                return  
        relay_content = self.delivery_content_switch(delivery['delivery_content'],msg)
        mainlogger.info('--告警转发内容 :   %s'%relay_content)
        ...
        self.insert_delivery_record(msg,param,relay_content)
