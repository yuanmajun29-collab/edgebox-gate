...
def queryControlTaskInfo():
    ...
    response['createTime'] = control_manage['create_time'].strftime("%Y-%m-%d %H:%M:%S")
    response['deviceList'] = []
    ...
    for item in model_associate:
        ...
        iter["createTime"] =int(item["create_time"].timestamp()) *1000
...
def queryControlTaskList():
    ...
    for control_item in control_items:
        ...
        for algorithm_item in algorithm_associate:
            ...
            alitem['createTime'] = int(algorithm_item['create_time'].timestamp())
...
def queryItcServer():
    ...
    if itc_item:
        itc_entity = {'soundType':'1',
                      ...
                      'itcServerPort':str(itc_item['itc_server_port']),  
    ...
    if lins_item:
        lings_entity = {'soundType':'2',
                        ...
                        'itcServerPort':str(lins_item['lings_server_port']),  
...
def testItcServer():
    ...
    try:
        ...
        if soundType == '1':
            ...
            #itc音响
            server_url = 'http://%s:%s'%(itcServerAddress,itcServerPort)
            lings_instance = LingsSound(tts_url=None,server_url=server_url,sound_no=None,volume=70)
        ...
        mainlogger.debug("*****音响服务器无法访问,Error :%s"%e)
...
@bp.route('/recvhot', methods=['POST','GET'])
def recvhot():
    '''
    接口说明:接受热成像摄像头的告警
    '''
    req_content = (request.form,request.files)
    mainlogger.debug('--原始表单: %s'%req_content[0])

    from algorith_server.AlgorithServer_new import RecvHikHotThread
    sender = RecvHikHotThread(context=None,req=req_content)
    sender.run()
