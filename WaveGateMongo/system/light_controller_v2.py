...
from system.mongo_search_v2 import recognize_road_conditions
...
class TrafficLightClient:
    ...
    def connect(self, retry_delay=1, max_retries=50):
        ...
        while retry_times < max_retries:
            try:
                ...
                retry_times += 1
                cross_logger.debug(
                    "红绿灯 {}:{} 连接失败：{}，将在{}秒后第{}次重试...".format(self.host, self.port, e, retry_delay,
                                                                              retry_times))
    ...
    def send_and_recv(self, cmd, max_retries=20):
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                cross_logger.debug(
                    "----------第{}次尝试发送命令：{}到{}:{} 接收超时---------------".format(attempt + 1, cmd_str,
                                                                                            self.host, self.port))
            except Exception as e:
                cross_logger.debug(
                    "----------第{}次尝试发送命令：{}到{}:{}通讯失败：{}---------------".format(attempt + 1, cmd_str,
                                                                                              self.host, self.port, e))
            if attempt == max_retries - 1:
                cross_logger.debug(
                    "----------第{}次尝试发送命令,{}到{}:{}达到最大重试次数，重新连接---------------".format(attempt + 1,
                                                                                                            cmd_str,
                                                                                                            self.host,
                                                                                                            self.port))
                self.connect()
    ...
    def set_light(self, color, mode):
        ...
        cross_logger.debug(
            "尝试设置地址 {}:{} 的灯光: 【{} | {}】".format(self.host, self.port, color_intro.get(color),
                                                          mode_intro.get(mode)))
...
# 关闭多个方向的红绿灯
def close_multi_lights_directions(clients, close_directions):
    handle_sockets = [clients[direction] for direction in clients if direction in close_directions]
    for handle_socket in handle_sockets:
        handle_socket.close_light_thread()


# 打开多个方向的红绿灯
def set_multi_lights_directions(clients, light_seconds=0, green_directions=None, red_directions=None,
                                yellow_directions=None):
    # 未设置灯和方向，直接返回
    if not (green_directions or red_directions or yellow_directions):
        return
    # 先判断控制红灯的逻辑，再判断控制黄灯的逻辑，最后判断绿灯的逻辑
    if red_directions and isinstance(red_directions, list):
        handle_sockets = [clients[direction] for direction in clients if direction in red_directions]
        for handle_socket in handle_sockets:
            handle_socket.open_light_thread(1, 1)
    if yellow_directions and isinstance(yellow_directions, list):
        handle_sockets = [clients[direction] for direction in clients if direction in yellow_directions]
        for handle_socket in handle_sockets:
            handle_socket.open_light_thread(2, 0)
    if green_directions and isinstance(green_directions, list):
        handle_sockets = [clients[direction] for direction in clients if direction in green_directions]
        for handle_socket in handle_sockets:
            handle_socket.open_light_thread(0, 1)
...
def smart_mode_pre_recognition(clients):
    ...
    condition = HandleRoadCondition().query_current_road_state()
    ...
    cross_logger.debug("=============雷达数据分析结果：{}".format(condition))
    road_condition_now = condition.get('road_condition')
    # 判断当前是否处于省电模式
    power_mode = False
    vehicle_directions = list()
    # 识别到人
    if condition.get("pedestrian"):
        road_condition_now = "has_pedestrian"
    # 近点/远点摄像头识别到车，或者雷达识别到车
    else:
        near_vehicles = condition["camera_near_vehicles"]
        far_vehicles = condition["camera_far_vehicles"]
        radar_vehicles = condition["radar_vehicles"]
        all_vehicle_directions = near_vehicles + far_vehicles + radar_vehicles
        vehicle_directions = list(set(all_vehicle_directions))
        if len(vehicle_directions) > 1:
            road_condition_now = "multi_direction_vehicles"
        elif len(vehicle_directions) == 1:
            road_condition_now = "one_direction_vehicle"
        else:
            road_condition_now = "no_pedestrian_no_vehicle"

    cross_logger.debug("----------road_condition_now{}----------".format(road_condition_now))
    return {
        "road_condition": road_condition_now,
        "directions": vehicle_directions
    }
...
def run_smart_mode(clients, ruler):
    ...
    last_vehicle_directions = list(
        map(lambda last_vehicle_direction: last_vehicle_direction.decode(), last_vehicle_directions))
    cross_logger.debug("前一次路况: {}, 本次路况: {}".format(last_road_condition, road_condition))
    cross_logger.debug("last_vehicle_directions: {}, directions: {}".format(last_vehicle_directions, directions))
    yellow_wait_time = 4
    if road_condition == 'has_pedestrian':
        cross_logger.debug('检测到有行人')
        set_multi_lights_directions(clients, pedestrian_traffic_time, yellow_directions=[1, 2, 3, 4])
    # 一个方向有来车
    elif road_condition == 'one_direction_vehicle':
        vehicle_direction = directions[0]
        cross_logger.debug('检测到{}方向有来车'.format(vehicle_direction))
        # 判断该方向上一次是否也为有车
        if last_road_condition == "one_direction_vehicle":
            if last_vehicle_directions[0] == vehicle_direction:
                set_multi_lights_directions(clients, vehicle_traffic_time, green_directions=[vehicle_direction])
            else:
                # 前一个方向黄闪4s，之后前一个方向设置为红灯，当前方向绿灯
                set_multi_lights_directions(clients, yellow_wait_time, yellow_directions=last_vehicle_directions)
                set_multi_lights_directions(clients, vehicle_traffic_time, red_directions=last_vehicle_directions,
                                            green_directions=[vehicle_direction])
        elif last_road_condition == "multi_direction_vehicles":
            # 将非该车方向的红绿灯设置为黄闪
            yellow_directions = list(set(last_vehicle_directions) - {vehicle_direction})
            set_multi_lights_directions(clients, yellow_wait_time, yellow_directions=yellow_directions)
            set_multi_lights_directions(clients, vehicle_traffic_time, red_directions=yellow_directions,
                                        green_directions=[vehicle_direction])
        else:
            red_directions = list({1, 2, 3, 4} - {vehicle_direction})
            set_multi_lights_directions(clients, vehicle_traffic_time, red_directions=red_directions,
                                        green_directions=[vehicle_direction])
        redis_database.lpush('vehicle_directions', *directions)
    elif road_condition == 'multi_direction_vehicles':
        cross_logger.debug('检测到{}方向有来车'.format(",".join(directions)))
        if last_road_condition == "one_direction_vehicle":
            not_match_directions = list(set(last_vehicle_directions) - set(directions))
            if not_match_directions:
                set_multi_lights_directions(clients, yellow_wait_time, yellow_directions=not_match_directions)
                set_multi_lights_directions(clients, 0, red_directions=not_match_directions)
        elif last_road_condition == "multi_direction_vehicles":
            not_match_directions = list(set(last_vehicle_directions) - set(directions))
            if not_match_directions:
                set_multi_lights_directions(clients, yellow_wait_time, yellow_directions=not_match_directions)
        ...
        cross_logger.debug('未检测到人和车')
        redis_database.delete('vehicle_directions')
        # 检查省电模式是否开启
        power_save = False
        if power_save:
            # 方向1黄闪，其他方向灯灭
            set_multi_lights_directions(clients, 0, yellow_directions=[1])
            close_multi_lights_directions(clients, [2, 3, 4])
        else:
            # 进行四灯黄闪10S
            pedestrian_traffic_time = ruler.get('pedestrian_traffic_time')
