...
SYNC_DYNAMIC_STATUS = "/device/syn/synDynamicDeviceStatus"
SYNC_DYNAMIC_EMERGENCY = "/business/syn/emergency"
SYNC_TEMPERATURE = "/business/syn/synTemperature"
...
def sync_device_status(url,data):
    ...
    mainlogger.debug("--动环设备状态同步:%s"%resp.json())
    return
...
def sync_temperature(url,content):
    try:
        ...
        headers = {'Content-Type': 'application/json'}
        resp = requests.post(url,data=json.dumps(content),headers=headers,verify=False)
        mainlogger.debug("--动环温湿度上报:%s;\n--温度上报入参：%s"%(resp.json(),content))
    except Exception as e:
        mainlogger.debug("--动环温湿度上报error:%s"%e)
    return
...
def insert_to_database(db:ToMongo,item:dict()):
    try:
        ...
        mainlogger.debug('insert_to_database error:%s'%e)
...
def sound_control(db:ToMongo,item):
    try:
        ...
        mainlogger.debug("动环音箱告警失败,error:%s"%e)
...
def deal_msg(db:ToMongo,rePool,conn,socketMacMap,mode,url,msg):
    try:
        ...
        for i in range(4):
            ...
            if not item:
                mainlogger.debug("--无动环设备绑定，query：%s"%query)
                #如果无动环设备绑定，则直接返回
                continue
        ...
        mainlogger.debug("动环deal_msg error:%s"%traceback.format_exc())
...
class KNDserver:
    ...
    def accept_conn(self):
        while True:
            ...
            mainlogger.debug("--接收到康耐德连接请求： %s"%str(conn.getpeername()))
            self.conn_list = []
            self.conn_list.append(conn)

    def recv_message(self):
        mainlogger.debug("--接收康耐德告警信息--")
        while True:
                ...
                for conn in self.conn_list:
                    while True:
                        try:
                            ...
                            mainlogger.debug("--接收的康奈德信息，ip:%s,message:%s"%(ip,result))                            
                            self.socketMacMap = deal_msg(self.my_db,self.rePool,conn,self.socketMacMap,self.mode,self.url,result)
                        except Exception as e:
                            self.conn_list.remove(conn)
                            mac = self.socketMacMap.get(ip)
                            mainlogger.debug("error:%s,客户端下线了,ip:%s,mac:%s"%(e,ip,mac))
                            continue
    ...
    def check_status(self):
        try:
            ...
            mainlogger.debug("动环check_status_error :%s"%e)
        finally:
