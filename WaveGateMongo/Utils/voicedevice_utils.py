...
class VoiceBoxUtils:
    ...
    def login(self):
        ...
        self.token = token
        mainlogger.debug("token :%s"%token)
        return jsonobject
    ...
    def refresh(self):
        ...
        mainlogger.debug('refresh:%s'%result)
        # print(result.json())
        return result
    ...
    def getterminalinfo(self):
        ...
        mainlogger.debug('getterminalinfo: %s'%result)
        return result
    ...
    def playTTS(self,endPointsList,content):
        ...
        mainlogger.debug('playTTS: %s'%result)
        return result
    ...
    def getTTS(self):
        ...
        header = {"Content-Type": "application/json"}
        result = requests.put(self.url + "/api/v29+/ws/forwarder", json.dumps(params),headers=header,verify=False)
        mainlogger.debug('getTTS: %s'%result)
        return result
    ...
    def getMusicList(self):
        ...
        header = {"Content-Type": "application/json"}
        result = requests.put(self.url + "/api/v29+/ws/forwarder", json.dumps(params),headers=header,verify=False)
        mainlogger.debug('getMusicList: %s'%result)
        return result
    ...
    def playMusic(self,endPointIDsList,musicIDsList):
        ...
        mainlogger.debug('playMusic: %s'%result)
        return result

class LingsSound:


    def __init__(self,tts_url,server_url,sound_no,volume):
    ...
    def login(self,account,password):
        ...
        params['user'] = data
        header = {"Content-Type": "application/json"}
        result = requests.post(login_url,headers=header,data=json.dumps(params),timeout=5).json()       
        return result
    ...
    #         mainlogger.debug('LinsSound playtts : %s'%result.json())
    ...
    def playTTSByServer(self,content):
        mp3_url = "%s/tts_xf.single?text=%s&voice_name=xiaoyan&speed=50&volume=100"%(self.tts_url,content)
        data = dict()
        data['app'] = "ls20"
        data['callid'] = "1656901436"
        data['type'] = "req"
        data['name'] = "songs_queue_append"
        data['sns'] = [self.sound_no]
        params = dict() 
        params['tid'] = "234"
        params['vol'] = self.volume
        params['urls'] = [{'name':'告警语音.mp3','uri':mp3_url}]
        ...
        header = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Trident/7.0; rv:11.0) like Gecko',"Content-Type": "application/json"}
        url = "%s/api/sendtodevice"%(self.server_url)
        try:
            result = requests.post(url,json.dumps(data),headers=header,verify=False)
            mainlogger.debug('LinsSound playtts : %s'%result.json())
        except requests.exceptions.Timeout or requests.exceptions.ConnectionError:
            time.sleep(3)
            result = requests.post(url,json.dumps(data),headers=header,verify=False)
...
def sendSoundmessage(sound_col,equip_id,sound_type,sound_file,server_url=None,account=None,password=None,tts_url=None):
    try:
        sound_id_list = equip_id.split(',')
        for sound_id in sound_id_list:
            query = {'sound_id':sound_id}
            sound_item = sound_col.find_one(query)
            sound_status = sound_item.get('sound_status',None)
            if sound_status == '1':
                #音响设备下线
                continue
            device_type = sound_item.get('sound_type',None)
            if not sound_file:
                return
            if device_type == '1':
                #设备类型为itc音响
                ...
                if sound_type == 1:
                    #tts
                    tts_result = VoiceInstance.playTTS(endPointsList=1,content=sound_file)
                    tts_result = tts_result.json()
                    if tts_result.get('result') != 200:
                        mainlogger.debug('Error : 播放tts失败!')
                elif sound_type == 2:
                    #音频文件
                    music_result = VoiceBoxUtils.playMusic(endPointIDsList=[1],musicIDsList=[sound_file])
                    music_result = music_result.json()
                    if tts_result.get('result') != 200:
                        mainlogger.debug('Error : 播放音乐失败!')
            elif device_type == '2':
                #设备类型为菱声音响
                sound_no = sound_item.get('sound_no',None)
                Lingsinstance = LingsSound(tts_url,server_url,sound_no,volume=70)
                if sound_type == 2:
                    #tts
                    tts_result = Lingsinstance.playTTSByServer(content=sound_file).json()
                    if tts_result.get('code') != 0:
                        mainlogger.debug('Error : 播放tts失败!')

            mainlogger.debug("sendSoundmessage  tts_result%s"%tts_result)
