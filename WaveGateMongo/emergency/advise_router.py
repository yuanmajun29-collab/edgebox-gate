...
def addSmsDelivery():
    ...
    item={"sms_delivery_id":sms_delivery_id,"sms_delivery_name":smsDeliveryName,"delivery_content":deliveryContent,
          "delivery_phones":deliveryPhones,"trigger_condition":triggerCondition,"organization_id":organization_id,
...
def retrySmsDelivery():
    ...
    if  delivery_item:
        ...
        if result == '1':
            my_db.update('odin_advise_sms_delivery_record',{'delivery_record_id':smsDeliveryRecordId},{"$set":{'delivery_status':'1'}})
            response_data['requestStatus'] = "SUCCESS"
        else:
            response_data['requestStatus'] = "FAIL"
    else:
...
def addWebhookDelivery():
    ...
    item={"webhook_delivery_id":webhook_delivery_id,"webhook_delivery_name":webhookDeliveryName,"webhook_delivery_address":webhookDeliveryAddress,
          "request_type":requestType,"request_headers":requestHeaders,"frequency_type":frequencyType,"frequency_date_format":frequencyDateFormat,
          "frequency_count":frequencyCount,"timeout":timeout,"trigger_condition":triggerCondition,"delivery_content":deliveryContent,
          "create_time":create_time,"update_time":update_time,
          "control_name":controlName,"device_name":deviceName,"model_path":modelPath,
...
def getWebhookDeliveryDetail():
    ...
    if webhookDeliveryId:
        webhook_item = my_db.get_col("odin_advise_webhook_delivery").find_one({"webhook_delivery_id":webhookDeliveryId})
...
def retryWebhookDelivery():
    ...
    if  delivery_item:
        ...
        result = sms_sender.resend_sms_delivery(delivery_item)
        if result == '1':
            my_db.update('odin_advise_sms_delivery_record',{'delivery_record_id':webhookDeliveryRecordId},{"$set":{'delivery_status':'1'}})
            response_data['requestStatus'] = "SUCCESS"
        else:
            response_data['requestStatus'] = "FAIL"
    else:
...
def testWebhook():
    ...
    try:
       # a = request.get_json()
        b = request.data
        mainlogger.debug('原始内容 : %s'%b)
    except:
        import traceback
        mainlogger.debug('内容格式不对 %s'%traceback.format_exc())
