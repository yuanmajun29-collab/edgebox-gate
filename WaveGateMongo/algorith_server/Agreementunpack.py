            ...
            if num == -1:
                ...
                if len_msg >= 55:
                    ...
                    if len_msg >= length:
                        msg_body = msg_cache[55:length]
                        msg_type = msg_cache[18:23]
                        mainlogger.debug('--msg_type : ' + msg_type)
...
def check_and_save_crossroads_algo_msg(my_db, device_id, identifier, target_list, alg_num, emergency_time_stamp, send_time_stamp):
    try:
        # todo 检查前先确认是否在智能通行时间范围内
        mainlogger.debug("===============alg数据写入：{}".format(target_list))
        mainlogger.debug("========device_id:{} alg_num: {}".format(device_id, alg_num))
        if check_crossroads_algo_msg(my_db, device_id, alg_num):
            ...
            mainlogger.error("===============alg_num：{} pedestrian_alg_num: {}".format(alg_num, pedestrian_alg_num))
            if alg_num == pedestrian_alg_num:
                ...
                roi_vehicle_near_id = 1
                roi_id_list = get_unique_roi_ids(target_list, identifier)
                for roi_id in roi_id_list:
                    if roi_id == roi_vehicle_near_id:
                        alert_type = 'vehicle_near'
                    else:
                        alert_type = 'vehicle_far'
                    data = {
                        'device_identification': device_id,
                        'alert_type': alert_type,
                        'alert_time': alert_time_utc,
...
def thread_handle_crossroads_algo_msg(my_db, device_id, identifier, target_list, alg_num, emergency_time_stamp, send_time_stamp):
