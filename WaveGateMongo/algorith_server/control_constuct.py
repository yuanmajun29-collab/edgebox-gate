...
def switch_roi(roi_item):
   ...
   # mainlogger.debug('roi_area_info： %s'%roi_info_item)
...
def get_camera_name(cameraItems):
    name = cameraItems.get("camera_name")
    if not name:
        name = 'camera name empty'
    return name
...
class ControlSqlHelperv2():
    ...
    def build_controls_message(self):
        ...
        if controlMissons:
            for controlItem in controlMissons:  
                ...
                for missionDevice in missionDevices:
                    ...
                    item['device_name']=get_camera_name(cameraItems)
                    ...
                    for match_model in match_models:
                        try:
                            ...
                            query_roi = {"algorithm_constant_id":{"$regex":service_num},
                                        "camera_id":device_id}
                            roi_items = roi_col.find(query_roi)
                            ...
                            continue
                        #只有睡岗和离岗有rate_num +车通道占用
                        if constant_num in ['113','114','161']:
                            try:
                                rate_num = constant_item['rate_num']
                                iter["rate_num"] = float(rate_num) if rate_num else None  
                            except:
                                iter["rate_num"] = None
                        else:
                            iter["rate_num"] = None
                        algorithm_interval = constant_item.get("algorithm_interval")
                        iter["duration_num"] = int(algorithm_interval) if algorithm_interval else None
                        iter["roi_list"] = []
                    #    mainlogger.debug('roi_items:%s'%roi_items)
                        for roi_item in roi_items:
                            roi_alg_list = roi_item['algorithm_constant_id'].split(',')
                    #        mainlogger.debug('service_num:%s,roi_alg_list:%s;'%(service_num,roi_alg_list))
                            if service_num in roi_alg_list:
                                iter['roi_list'].append(switch_roi(roi_item))
                        iter["person_list"] = []
                        #目标布控才下发人脸
                        if constant_num == "100" or constant_num == "173":
        ...
        # try:
        #     #构建人流统计布控任务
        #     crowd_col = self.my_db.get_col('crowd_entrance_camera_associate')
        #     cameraId_list = crowd_col.distinct("camera_id")
        #     devicesDict = mission_devices(items) 
        #     keyList = devicesDict.keys()
        #     crowdAlgStr = constant_to_str(constant_num="108")
        #     for cameraId in cameraId_list:
        #         if cameraId in keyList:
        #             index = devicesDict[cameraId]
        #             item = items[index]
        #             alg_param = item['algorithm']                   
        #             item['algorithm'] = constant_to_str("108",alg_init=alg_param)
        #             algorithmListItem = dict()
        #             algorithmListItem["algorithm"] = crowdAlgStr
        #             algorithmListItem["person_list"] = []
        #             algorithmListItem["rate_num"] = None
        #             algorithmListItem["duration_num"] = None
        #             algorithmListItem["roi_list"] = []
        #             roi_items = roi_col.find({"roi_type":"2","camera_id":cameraId})
        #             for roi_item in roi_items:
        #                 algorithmListItem['roi_list'].append(switch_roi(roi_item))
        #             item["algorithm_list"].append(algorithmListItem)
